...
// GET /api/mcp/role - List role-scoped MCP connections for the user's role (sanitized, read-only)
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('Error fetching role MCP connections:', error);
    return NextResponse.json(
      { error: 'Failed to fetch role MCP connections' },
      { status: 500 }
    );
  }
}
