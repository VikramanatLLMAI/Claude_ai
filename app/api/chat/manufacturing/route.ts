...
export async function POST(req: Request) {
  try {
    ...
    console.log(`[${SOLUTION_TYPE}] activeMcpIds received:`, JSON.stringify(activeMcpIds));
    if (activeMcpIds && activeMcpIds.length > 0) {
      console.log(`[${SOLUTION_TYPE}] Loading MCP tools from ${activeMcpIds.length} connections:`, activeMcpIds);
      try {
        const { tools: mcpTools, descriptions } = await loadActiveMcpToolsWithDescriptions(activeMcpIds);
        const mcpToolCount = Object.keys(mcpTools).length;
        console.log(`[${SOLUTION_TYPE}] MCP tools loaded: ${mcpToolCount}`, Object.keys(mcpTools));
        if (mcpToolCount > 0) {
          Object.assign(tools, mcpTools);
          mcpToolDescriptions = descriptions;
          console.log(`[${SOLUTION_TYPE}] Added ${mcpToolCount} MCP tools to tools object`);
          console.log(`[${SOLUTION_TYPE}] MCP tool descriptions for prompt:`, descriptions.map(d => d.name));
        } else {
          console.log(`[${SOLUTION_TYPE}] No MCP tools returned - check connection status and availableTools`);
        }
      } catch (error) {
        console.error(`[${SOLUTION_TYPE}] Error loading MCP tools:`, error);
      }
    } else {
      console.log(`[${SOLUTION_TYPE}] No activeMcpIds in request, skipping MCP tool loading`);
    }
    ...
    console.log(`[${SOLUTION_TYPE}] System prompt includes ${mcpToolDescriptions.length} MCP tool descriptions`);
    ...
    console.error('Manufacturing Chat API error:', error);
    return new Response(
      JSON.stringify({
