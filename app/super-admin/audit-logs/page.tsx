          ...
          <div className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                ...
                <p className="mt-0.5">{log.organization?.name ?? "Platform"}</p>
              </div>
            </div>
            <div>
...
export default function AuditLogsPage() {
  ...
  // ---- Fetch filter meta (actions, users, orgs) once ----
  React.useEffect(() => {
    async function fetchMeta() {
      try {
        const res = await fetch("/api/super-admin/audit-logs?meta=true", {
  ...
  const buildParams = React.useCallback(
    ...
    [page, pageSize, sortBy, sortOrder, startDate, endDate, orgFilter, actionFilter, userFilter]
  )
  ...
  }, [sortBy, sortOrder, startDate, endDate, orgFilter, actionFilter, userFilter, pageSize])
  ...
  const buildExportUrl = (format: "csv" | "json") => {
    ...
    if (actionFilter) params.set("action", actionFilter)
    if (userFilter) params.set("userId", userFilter)
    return `/api/super-admin/audit-logs/export?${params.toString()}`
  }
  ...
  const hasFilters = startDate || endDate || orgFilter || actionFilter || userFilter
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-foreground">
                        {log.organization?.name ?? (
                          <span className="text-muted-foreground italic text-xs">Platform</span>
                        )}
                      </td>
